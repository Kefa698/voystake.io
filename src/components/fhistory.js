...
const ListItem = ({sort, id, date, amount, percent, dateHead}) => {
    let fee = Number(fromWEI(percent));
    console.log('amount', amount);
    console.log('fee', fee);
    // percent = fee / (Number(amount) + fee) * 100;
        ...
        <div className="list history-web">
            ...
            <div className="row list-item mt-2">
                ...
                <div className="col-md-3 f-j-center">
                    <h5>{percent !== "NaN"? percent : ''}</h5>
                    <h6 className='c-g-1 fs-12'>{percent !== "NaN"? 'Fee' : ''}</h6>
                </div>
                
                <div className="col-md-3 f-j-center">
                    <h5>{numberWithCommas(amount)}</h5>
                    <h6 className='c-g-1 fs-12'>VOY</h6>
                </div>
            </div>
        </div>
            ...
            <div className="row list-item mt-2 fw-nw">
                <div className="col-md-4 part1">
                    <div className='px-2 d-flex'>
                        <div className='pt-1'>
                            {/*<img className='hl-img' src={StakingImg} width={20} height={20} alt="stakeImg" />*/}
                        </div>
                ...
                <div className="col-md-4 part3">
                    <div className="f-i-center">
                        ...
                        <div className='ms-2 fs-10'>VOY</div>
                    </div>
    ...
    const get_history = (history, sort = "stake") => {
        ...
        if (array_history.length === 0) {
          return (
            ...
            console.log('array_history', array_history);
          let date = [];
          let headDate = [];
          ...
          return array_history.map((value, index, history_array) => {
            ...
            return (
                <ListItem key={index} sort={sort} id={index+1} date={date[length-index]} amount={amount} percent={array_history[length - index].returnValues.unStakeFee} dateHead={headDate[length-index] !== headDate[length-index+1] ? headDate[length-index] : ""} />
            );
          });
        }
    }
                ...
                <div className="row rate-mobile fw-nw">
                    <div className='col-md-4 p-4'>
                        <div className='row p-3 b-b-2 br-8 fw-nw'>
                            <div className='col-md-4'>
                                <h1>{numberWithCommas(props.stakePool)}%</h1>
                                <h6 className='c-g-1'>Staked in Pool</h6>
                            </div>
                            <div className='col-md-1'>
                                <ChartStaking width={numberWithCommas(props.stakePool)}/>
                            </div>
                        </div>
                        ...
                        {/* <div> */}
                            <div className='row p-3 b-b-2 br-8 stake-pool'>
                                <div className='col-md-9'>
                                    <h1>{props.connectState ? numberWithCommas(props.stakePool) : "-"}%</h1>
                                    <h6 className='c-g-1'>Staked in Pool</h6>
                                </div>
